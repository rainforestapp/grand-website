...
After queueing the signup, `index.html` stores the email in `sessionStorage` under `grand_signup_email` and redirects to `welcome.html`, a `noindex` page that collects four optional qualifying fields: `full_name`, `zipcode`, `reason_interested`, and `lives_alone` (yes/no/not_sure). It POSTs a `{ type: "waitlist_profile", email, ... }` payload to the same endpoint. The alpha tester question is no longer a yes/no field — instead the page invites interested visitors to book an onboarding call via Calendly (`https://calendly.com/jamescpalmer/grandalpha`), so no `alpha_tester` value is collected from the form anymore. The backend still tolerates the field for older submissions.
