...
- **Moved the alpha-tester Calendly link to the post-submit Thank-you screen** on `welcome.html`, out of the profile form. **Why:** the scheduling CTA was competing with the profile questions; showing it only after submit keeps the form focused and puts the invite at the natural next step.
...
After queueing the signup, `index.html` stores the phone number in `sessionStorage` under `grand_signup_phone` and redirects to `welcome.html`, a `noindex` page that collects four optional qualifying fields: `full_name`, `zipcode`, `reason_interested`, and `lives_alone` (yes/no/not_sure). It POSTs a `{ type: "waitlist_profile", product: "grandphone", phone, ... }` payload to the same endpoint. The alpha tester question is no longer a yes/no field — instead, the Thank-you screen shown after the profile form is submitted invites interested visitors to book an onboarding call via Calendly (`https://calendly.com/jamescpalmer/grandalpha`), so no `alpha_tester` value is collected from the form anymore. The backend still tolerates the field for older submissions.
