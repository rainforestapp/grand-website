...
After queueing the signup, `index.html` stores the phone number in `sessionStorage` under `grand_signup_phone` and redirects to `welcome.html`, a `noindex` page that collects four optional qualifying fields: `full_name`, `zipcode`, `reason_interested`, and `lives_alone` (yes/no/not_sure). It POSTs a `{ type: "waitlist_profile", product: "grandphone", phone, ... }` payload to the same endpoint. The alpha tester question is no longer a yes/no field — instead, the confirmation screen shown after the profile form is submitted (headed "Fast track getting your alpha device") invites the tester to book an onboarding call via a primary "Schedule a 20-minute call with us" button linking to Calendly (`https://calendly.com/jamescpalmer/grandalpha`), so no `alpha_tester` value is collected from the form anymore. The backend still tolerates the field for older submissions.
